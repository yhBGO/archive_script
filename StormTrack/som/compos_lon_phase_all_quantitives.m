...
%	for p=1
	...
	for	lonRef_actral=[lon_phase_mean(:,p)]';	
...
%	text(double(min(xC(:))+1),double(min(yC(:))+30),['(cint: ', 'h ', num2str(cnintC2), 'm ', ')'],'fontsize',15);
	...
	set(gcf, 'Papersize',[800,800])
 	set(gcf, 'Position', [0 0 800 800]);
		...
		if ~isdir([somdata,'/lon_phase_all_lat/'])
		mkdir([somdata,'/lon_phase_all_lat/'])
		end

% 	orient Landscape
% 	saveas(gcf,[somdata,'/lon_phase_all_lat/,'composition_',num2str(p),'lon',num2str(lonRef).pdf'],'pdf')
%	orient portrait
	saveas(gcf,[somdata,'/lon_phase_all_lat/','composition_',num2str(p),'lon',num2str(lonRef), '.eps'],'psc2')
