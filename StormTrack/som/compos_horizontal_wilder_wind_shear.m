...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%===============================================
    ...
    dataC=dataC(1:73,37:end,find(datalevelC==100),:)-dataC(1:73,37:end,find(datalevelC==300),:);
...
%	dataC=dataC(find(datalonC==20):find(datalonC==130),:,:,:);	
    ...
    mkdir([somdata,'/','wind_shear'])

	save([somdata,'/','wind_shear','/Cps_','wind_shear','_on_S_',somdata,'.mat'],'composition','datalonC','datalatC','datalevelC','pattern_num','Composdata','somdata')
...
%*************************************************************
	...
	text (double(min(datalonC)+1),double(max(datalat)+5),['(',num2str(p),')',sprintf('%3.1f',100*pat_f(p)),'%'],'fontsize',9)
    	text(double(min(datalonC)+(max(datalonC)-min(datalonC))/2.0),double(max(datalat)+5),['lev:',num2str(levelS)],'fontsize',9)
...
%======save figure============================
 ...
 figname=['wind_shear_100-300_',Composdata,'_base_SOM_',somdata]
 figdir= ['wind_shear']
