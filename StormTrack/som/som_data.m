...
X = double(reshape(x_weight,length(datalon)*length(datalat),length(time_YYYYMMDD)));
	...
	%som
	[pattern,pat_f,timeseies]=som(X',ssd*(yrEnd-yrStrt),yrStrt,yrEnd,[1:JFM,365-ND+1:365])
...
% 	saveas(gcf,[name,'/plot_som_',name,'.pdf'],'pdf')
% close


%save som result
    	save([name,'/som_',name,'.mat'],'Map_cluster','pattern','pat_f','pat_f_ND','pat_f_FM','timeseies','datalon','datalat')%,'dataplev')
