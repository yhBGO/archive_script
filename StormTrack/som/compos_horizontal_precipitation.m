	...
	levelS=250;
       ...
       %% composition of other quantities
     	Composdata='precip_NDJFM_lev0_lat0-90_lon20-130_year1979-2010_filter-'
	...
	%Composdata='w_NDJFM_lev1000-10_lat0-90_lon20-130_year1979-2010_filter-deseasonal3-highpassNwgt31-0.1'
     ...
     cnintC=0.5%(0.2 air)%(10-height)%(2uwind_pri)%(0.002omega)%(2-vwnd);
     cnintS=2;
 	color_plot=load('nrl_sirkes_nowhite.rgb');%'hotcold_18lev.rgb')%nrl_sirkes_nowhite%('sunshine_diff_12lev.rgb');%'MPL_PuOr.rgb';load('BlWhRe.rgb');%'MPL_PuOr.rgb'
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%===============================================
    ...
    %------------------------------------------- 
     ...
     datalevelC = 1000;%ncread(fC,levelnameC);
...
%	dataC=dataC(find(datalonC==20):find(datalonC==130),:,:,:);	
	...
	composition(:,:,p) = squeeze(mean(dataC(:,:,ind),3)); % no level dimension;
    	end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%lon-lat plot%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	...
	display(['cint(recomended)',num2str((max(composition_org(:))-min(composition_org(:)))/25)])
	...
	%calculation
        for levelRef=1000%[50,250,300,100,400,500,600,700,850]% 250 200;850,50,10
	...
	for p = 1:K
    	...
    	ind = find(timeseies(:,3) == p);
	%composition		
	composition(:,:,p) = squeeze(mean(composition_org(:,:,p),4));
...
%*************************************************************
	...
	'xtick',datalonC(1):30:datalonC(end),'xticklabel',...
	datalonC(1):30:datalonC(end),'fontsize',8);
